  ...
  const { post ,type} = location.state!=null? location.state :{}
    ...
    const [inputs, setInputs] = useState(location.state==null?{
      ...
      const handleSubmit = async (e) => {
        ...
        try {
         const res= await axios.post(`https://sayed.onrender.com/api/${Type.type}`, inputs);
          navigate("/Admin");
          console.log(res);
        } catch (err) {
          setError(err.response.data);
            ...
            <h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
              ...
              <div>
                 ...
                 <div>
                  ...
                  <button type="submit"  class="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{location.state!=null?"تعديل":"أضف"}</button>
